...
function get_messages(data) {
    ...
    console.log(num_mess)
    if (num_mess+1 === 50) {
        $("#conversation").prepend("<div style='width: 100%; text-align: center'><button id='more' style='align-content: center'>MORE</button></div>")
    }
    $("#conversation").scrollTop($("#conversation")[0].scrollHeight);
}

$('#conversation').on('click', '#more', function () {
    $(this).parent().remove();
        ...
        success: function (data) {
            if (data.r === 'ok') {
                ...
                $.each(data.history, function (i, obj) {
                    ...
                    if (last_a !== obj[0]) {
                        last_a = obj[0];
                        to_append = to_append + '<div class="author">' + obj[0] + '</div>';
                    }
                    to_append = to_append + '<div title="' + obj[2].slice(0, -10) + '" class="message">' + obj[1] + '</div>';
                });
                $("#conversation").prepend(to_append)
                if (num_mess+1 === 50) {
                    $("#conversation").prepend("<div style='width: 100%; text-align: center'><button id='more' style='align-content: center'>MORE</button></div>")
                }
            } else {
